...
export async function createUser(input: CreateUserInput): Promise<ActionResult<{ id: string }>> {
  ...
  if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
    return { success: false, error: "Thiếu cấu hình SUPABASE_SERVICE_ROLE_KEY" };
  }
  ...
  if (existingError) {
    ...
    return { success: false, error: "Không thể kiểm tra người dùng hiện tại" };
  }
  ...
  // IMPORTANT: public.users.id references auth.users(id)
  // Admin must create auth user first (service role) to get a valid id.
  ...
  if (authError || !authCreate?.user) {
    ...
    return { success: false, error: "Không thể tạo tài khoản đăng nhập (Auth)" };
  }
