...
export interface Request {
  ...
  created_at: string;
  updated_at: string;
  /** Optimistic locking: increment on each update (per PRD Section 3.3.4) */
  version: number;
}
...
/** Rate limiting tracking (per PRD Section 3.2) */
...
/** Cron job execution log for idempotency (per PRD Section 3.7) */
...
export type Database = {
  public: {
    Tables: {
      ...
      rate_limits: {
        Row: RateLimit;
        Insert: Omit<RateLimit, "id"> & { id?: string };
        Update: Partial<Omit<RateLimit, "id">>;
      };
      cron_logs: {
        Row: CronLog;
        Insert: Omit<CronLog, "id" | "created_at"> & { id?: string };
        Update: never; // Cron logs should not be updated
      };
    };
    Views: {
