...
CREATE INDEX idx_request_items_item_name ON request_items USING GIN(to_tsvector('simple', item_name));
...
CREATE INDEX idx_request_comments_content ON request_comments USING GIN(to_tsvector('simple', content));
...
CREATE POLICY auth_logs_select ON auth_logs FOR SELECT USING (user_has_role(auth.uid(), 'admin'));
CREATE POLICY auth_logs_insert ON auth_logs FOR INSERT WITH CHECK (true);
